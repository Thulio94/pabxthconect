...
class AgentDashboardController extends Controller
{
    ...
    public function history(Request $request): JsonResponse
    {
        $agent = $request->session()->get('sip_agent');
        [$filters, $historyInfiniteEnabled] = $this->historyFilters($request);
        if (! $historyInfiniteEnabled) return response()->json(['calls' => [], 'next_cursor' => null]);
    ...
    private function historyFilters(Request $request): array
    {
        ...
        $normalizeDate = static function (mixed $value): ?string {
            ...
            try { return Carbon::createFromFormat('Y-m-d', $value)->toDateString(); } catch (\Throwable) { return null; }
        };
        ...
        if ($filters['from'] && $filters['to'] && Carbon::parse($filters['from'])->gt(Carbon::parse($filters['to']))) [$filters['from'], $filters['to']] = [$filters['to'], $filters['from']];
    ...
    private function historyQuery(array $agent, array $filters, bool $hasActiveFilters)
    {
        return CallRecord::query()->with('recording')->where('tenant_id', $agent['tenant_id'])->where('extension_id', $agent['extension_id'])
            ->when(! $hasActiveFilters, fn ($query) => $query->whereDate('started_at', today()->toDateString()))
            ->when($filters['from'], fn ($query, $from) => $query->whereDate('started_at', '>=', $from))
            ->when($filters['to'], fn ($query, $to) => $query->whereDate('started_at', '<=', $to))
