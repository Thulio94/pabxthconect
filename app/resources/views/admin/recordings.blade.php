...
<div class="app-shell"><x-sidebar />
    <main class="workspace admin-workspace recordings-workspace">
        ...
        <section class="registry recordings-registry"><div class="section-title"><div><p class="eyebrow">ARQUIVOS DISPONÍVEIS</p><h2>{{ $recordings->total() }} gravação(ões)</h2></div><span class="muted">Arquivos respeitam a retenção de cada empresa.</span></div>
            <div class="table-wrap"><table><thead><tr><th>Data</th><th>Empresa</th><th>Ramal</th><th>Telefone</th><th>Resultado</th><th>Duração</th><th>Ouvir</th></tr></thead><tbody>@forelse($recordings as $recording) @php($call = $recording->call)<tr><td>{{ $call?->started_at?->format('d/m/Y H:i') ?? '—' }}</td><td>{{ $call?->tenant?->name ?? '—' }}</td><td>{{ $call?->extension?->number ?? '—' }}</td><td>{{ $call?->to_number ?? $call?->from_number ?? '—' }}</td><td>{{ match($call?->status) { 'answered', 'completed' => 'Atendida', 'busy' => 'Ocupado', 'no_answer' => 'Sem resposta', 'cancelled' => 'Cancelada', 'failed' => 'Falhou', default => '—' } }}</td><td>{{ gmdate('H:i:s', (int) $recording->display_duration_seconds) }}</td><td><audio controls preload="none" src="{{ route('admin.recordings.play', $recording) }}">Seu navegador não suporta áudio.</audio></td></tr>@empty<tr><td colspan="7" class="empty-cell">Nenhuma gravação encontrada com estes filtros.</td></tr>@endforelse</tbody></table></div>
